...
1. 在 Unreal 中创建 **MtoU_LiveLink Binding**，指定 **Driver Skeletal Mesh**，
   ...
   **Refresh Preview**。Refresh 只读取 LOD0 源数据并生成 Actor 自有的瞬态数据，
   同时为 Driver 的每个 Morph 投射一个对应的 Morph Target；输入修改或源资产重建
   会将其标记为 Dirty，必须再次显式 Refresh。Refresh 是事务性的：任一必需 Morph
   无效时都不会留下局部生成的 Preview。
...
6. 在 **动画** 工作流中，使用互斥的 **实时预览** 和 **缓存播放** 模式进行
   复查。缓存播放要求连接已完成 Unreal 协商，会暂停实时采样并禁用实时传输
   上限。点击 **捕获并回放** 后，工具会按当前 Maya Playback Range（包含首尾帧）
   逐帧采样，把 protocol-v4 帧增量写入当前用户的系统临时目录，恢复原来的当前帧，
   并按捕获时的场景帧率一次性按序回放全部帧。捕获会显示当前/总进度，开始前自动
   停止 Maya 播放，并可点击取消。工具会在写入帧前估算临时磁盘用量，超过 1 GiB 时
   请求确认，空间不足时拒绝捕获；失败或取消都会删除未完成的缓存。
7. 回放期间状态会明确显示 Unreal 正在显示已捕获缓存，而不是当前 Maya 姿势。
...
协议版本 4 要求 Maya 和 Unreal 两端组件配套安装：`init` 携带所选的
