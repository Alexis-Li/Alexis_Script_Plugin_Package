...
6. 点击 **再次回放** 可重复使用已完成的缓存而不重新捕获。切回 **实时预览**
   会停止回放并立即提交 Maya 当前姿势。缓存播放不会创建 Unreal 资产；已完成缓存
   只保留在兼容的 Maya 会话中，并会在替换、取消、场景或角色变化、工具关闭或
   Maya 退出时删除。下次启动只会清理有效且超过 24 小时的 MtoU 缓存残留。
