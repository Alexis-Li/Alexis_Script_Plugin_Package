...
3. 在 Maya 中运行 `MtoULiveLink.py`，选择变形根骨骼，点击“设置角色”。
   同一行的按钮可手动选择 Display 控制器或选中重名骨骼。按钮下方的状态栏
   显示当前状态以及根骨骼、衣服、帧率和骨骼／BlendShape 数量。
4. 选择“动画”，点击“连接”，“断开”旁边的指示变绿。在 Maya 中摆姿、拖动
   时间轴或播放动画，即可在 Unreal 中查看效果。预览行的“上限”用于限制实时
   传输帧率。不可用按钮会用提示说明原因。同一连接的相同警告只打断一次，
   新错误仍会弹出；“诊断详情”位于最下方的警告开关旁。

## 动画预览

使用“动画”模式实时预览角色。需要查看一段缓存动画时，先设置 Maya 时间轴的
播放范围，再选择“缓存播放”并点击“捕获并回放”。捕获和上传完成后开始播放。
捕获相关按钮仅在“缓存播放”模式下出现在预览行下方。状态栏可区分捕获中、
上传中、回放中、完成停在最后一帧、已停止保留缓存和失败。
